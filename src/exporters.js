...
import { getBudgetAnalysis } from './budgetAnalyzer.js';
...
export function buildPdfBuffer(quote) {
  return new Promise((resolve, reject) => {
    ...
    if (budgetAnalysis) {
      cursorY += 88;
      ensurePageSpace(60);
      drawLabelValue(document, 'Draft Total (USD)', budgetAnalysis.draftUsdLabel, startX, cursorY, 220, { valueSize: 10 });
      drawLabelValue(document, 'Client Budget', budgetAnalysis.inputDisplay, startX + 220, cursorY, 180, { valueSize: 10 });
      drawLabelValue(document, 'Budget Status', budgetAnalysis.budgetStatusDisplay, startX + 420, cursorY, 180, { valueSize: 10 });
      cursorY += 26;
      drawLabelValue(document, 'Budget Advice', budgetAnalysis.budgetAdvice, startX, cursorY, 500, { valueSize: 9 });
      cursorY += 24;
    }
